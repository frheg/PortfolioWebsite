...
import { getActiveChapter, scrollToChapter, subscribeActiveChapter } from '../utils/chapterScroll'
...
function itemStateClass(isActive, featured) {
  if (featured) {
    return isActive
...
function ItemContent({ icon, label, featured, sticker }) {
  return (
    <>
...
export default function FloatingNav() {
  const activeChapter = useSyncExternalStore(subscribeActiveChapter, getActiveChapter, getActiveChapter)
            ...
            {navLinks.map((item) =>
              item.route ? (
                <NavLink
                  ...
                  className={({ isActive }) => `${ITEM_CLASS} ${itemStateClass(isActive, item.featured)}`}
                >
                  <ItemContent {...item} />
                </NavLink>
              ) : (
                <button
                  key={item.chapterKey}
                  type="button"
                  onClick={() => scrollToChapter(item.chapterKey)}
                  aria-label={item.label}
                  aria-current={activeChapter === item.chapterKey ? 'true' : undefined}
                  className={`${ITEM_CLASS} ${itemStateClass(activeChapter === item.chapterKey, item.featured)}`}
                >
                  <ItemContent {...item} />
                </button>
              )
